            ...
            if (self.uartState):
                try:
                    ...
                    if parts[0][:2] == "RB":
                        ...
                        data_with_num = f"{data} {pass_cmd_num}"
                        self.OutputText.insert(END,f"{data_with_num}\n", 'PASS_color')
                        self.OutputText.tag_config('PASS_color', foreground='blue')
                        self.OutputText.see(END) # Ensure the end of the text is visible
                        pass_cmd_num += 1
                    elif parts[0] == "PASS_1":
                        data_with_num = f"{data} {pass_cmd_num}"
                        data = data.replace('PASS_1','').strip()
                        self.OutputText.insert(END,f"{data_with_num}\n", 'PASS_color')
                        self.OutputText.tag_config('PASS_color', foreground='blue')
                        self.OutputText.see(END) # Ensure the end of the text is visible
                        pass_cmd_num += 1
...
def create_win_row(switch_dict, ser_frame_instance, parent, item_info, default_color, row_index ):
 ...
 # Add a button to the frame_row using grid
    ...
    # Extract the value from file_content
        ...
        label_2nd = Label(parent, textvariable=label_2nd_var, font=("Helvetica", 10), padx=2, pady=0, anchor='e', fg='RED', bg=default_color)
    else:
    ...
    def update_label(*args):
        ...
        try:
            ...
            match item_text[0:4]:
                ...
                case 'OP13':
                    cmd_to_ser(new_line,ser_frame_instance, f"SET ti_max_current {selected_option}")
                case 'OP14':
                    cmd_to_ser(new_line,ser_frame_instance, f"SET ti_resistor {selected_option}")
        except Exception as e:
    ...
    # Create a "READ" button for each row
        ...
        read_button = Button(parent, text="READ", command=lambda it=item_text, ev=entry_var, is_write=False:
                            button_pressed(it, ev, read_button, is_write), bd=0, relief=GROOVE, activebackground='#eeeeee', bg='#eeeeee')
        # Bind the <Button-1> event to the button_pressed function
...
def button_released(ser_frame_instance,row_index,item_text,label_2nd_widget, entry, entry_var, button, is_write, win_name, dim_id):
    ...
    ##########################################
    if is_write == True and not result == "r" and not result == "e" and not result == "n": # r is trim window 
    ...
    ##########################################
    elif not is_write and not result == "e" and not result == "n":         
    ...
    elif is_write == True and ( result == "e" or result == "n" ) : # win_trim and write  
        value = entry_var.get()
        action = "SET"
        if item_text[2:4] == 't1':
            ...
            HT2_NUM = value
        elif item_text[2:4] == 't4':
            string = 'TI_RESISTOR '+ value
        cmd_to_ser(new_line,ser_frame_instance, f"{action} {string}")
    elif not is_write and ( result == "e" or result == "n" ): # win_trim and write  
        action = "GET"
        if item_text[2:4] == 't1':
